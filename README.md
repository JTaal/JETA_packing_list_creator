...
Table of Contents
How to use	2
1.1	Make an order in OSPOS	2
1.2	Copy detailed description from OSPOS	6
1.3	Insert the detailed description into the order file	9
1.4	Open JETA_Packing_list_creator	10
1.5	Fill in all the necessary additional information and designated the correct pathways	12
1.6	Create the packing list	13
1.7	Check the packing list for mistakes	14
Troubleshoot	14
Required files:	15
Additional script/folder:	15
1. Packing list log.xlsx	16
2. Order file.xlsx	17
3. JETA Packing list customer info.xlsx	18
4. Mother-packing-list.docx	19
5. Recompile code	21
...
 
Figure 3: Adding in a customer name which can be selected

Figure 4: Adding an item to the shopping cart
 
Figure 5: Finishing the order by entering the register mode: invoice.
 
Figure 6: Finalising the order by adding the PO number in the comments and the invoice number. To generate the invoice and log the transaction press the invoice button.
...
Figure 7: Reports tab within OSPOS and the Detailed reports section with at the top the transactions tab
 
Figure 8: Within the detailed transactions report adjusting the Date Range to "All time" to reveal all the transactions ever performed
 
Figure 9: All the transactions that have been logged with their respective contents. To access the order description you'll have to press the + icon on the left side of the individual rows

Figure 10: The opened order that was made and all its contents being copied. This can be done both with and without headers just make sure that the order file headers are changed properly. Otherwise, you'll be able to find the dictionary with all the hard coded headers in the troubleshoot section.
 
...
if (type(record["Barcode"]) == str or type(record["Barcode"]) == float) and 
record["Barcode"] != "":
...
Troubleshoot:
1.	If you made changes to this document (especially the headers), it could be that it can’t be imported properly. Check whether the document you’re currently referencing to, contains the same headers as shown above. (MUST BE EXACTLY THE SAME SINDS KEYS ARE CHANGED INTO HASHES WHICH LOOK COMPLETELY DIFFERENT TO A PC)
2.	If changing the document didn’t fix your problem and you still can’t start the program. Then you should clear all the saved pathway settings (Code below or run CLEAR_PATHWAY_SETTINGS.PY) to allow the program to default back to empty strings. 
